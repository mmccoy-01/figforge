...
- [x] Focus the Connect Cloud interface on one Upload & Label workspace.
...
- [x] Non-destructive source-pixel crop with draggable edges/corners and reset.
- [x] Delete image.
...
- [x] Remove the local My Figures pane from the Connect Cloud interface.
- [x] Project thumbnail.
...
## Phase 8 — Revision History (removed from Connect Cloud UI)

- [x] Remove the server-only Save Version action and revision browser.
- [x] Use browser recovery for active drafts and `.figforge` files for durable checkpoints.
